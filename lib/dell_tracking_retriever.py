...
class DellTrackingRetriever(EmailTrackingRetriever):

  first_regex = r'.*href="(http[^"]*order-collection[^"]*)"'
  order_number_regex = r'(\d{9})'
  purchase_id_regex = r'(\d{13})''
  ...
  def get_order_number_from_email(self, raw_email):
    match = re.match(self.order_number_regex, raw_email)
    if match:
      return match.group(1)
    return ''

  def get_purchase_id_from_email(self, raw_email):
    match = re.match(self.purchase_id_regex, raw_email)
    if match:
      return match.group(1)
    return ''

  def get_subject_searches(self):
    return ["Your Dell Order Has Shipped for Dell Purchase ID:"]
  ...
  def get_items_from_email(self, email_str):
    ...
    order_prefix_span = soup.find("span", {"id": "QTY_0"})
